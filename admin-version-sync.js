// 管理员版本 - 寝室厨房打扫系统（实时同步版本）
class AdminVersionSync {
    constructor() {
        // 房间顺序
    ...
    async initFirebase() {
        try {
            ...
            if (connected) {
                ...
                console.warn("Firebase连接失败，使用本地存储模式");
                this.updateConnectionStatus('error');
                this.loadLocalState();
            }
        } catch (error) {
            console.error("Firebase初始化失败:", error);
            console.warn("使用本地存储模式继续运行");
            this.updateConnectionStatus('error');
            this.loadLocalState();
        }
    }
